...
//			accountlist.get(accountid);
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		//It was wrong because we checked if account exists and if it existed it threw notExist exception.
		//if (accountlist.containsKey(accountid))
		if (!accountlist.containsKey(accountid)) {
...
//			account.deposit(money);
	...
	public void transfer(String fromaccount, String toaccount, Money amount) throws AccountDoesNotExistException {
		//transfer from acc1 to acc1, not correct.
		//I found it on test when I tried to transfer to different account that does not exist
		// and it sill would transfer
